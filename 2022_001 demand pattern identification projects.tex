...
$L$ representa os prazos de entrega (IIDRVs, com média $\bar{L}$ e variância $var(L)$)), \\

$C_{i}^{2}$ representa o coeficiente de variação de $i$ \\
...
$\bar{i}$ é a média dos valores de $i$, e

$s$ é o desvio padrão \\

\begin{equation}
s = \sqrt{ \frac{1}{z} \sum_{i=1}^{z}{(i-\bar{i})^2}}
\end{equation}

Tal esquema foi posteriomente revisado por \cite{EavesKingsman2004}, considerando também o padrão de demanda irregular, conforme mostra a tabela apresentada abaixo.
...
alto  & alto  & alto  & altamente intermitente   \\ \cline {1-4}
\end{tabular}
